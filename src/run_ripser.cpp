...
std::vector<double> compute_pairs(std::vector<diameter_index_t>& columns_to_reduce, hash_map<index_t, index_t>& pivot_column_index,
                                  ...
                                  const binomial_coeff_table& binomial_coeff) {
  std::vector<double> persistence_pairs;
	...
	for (index_t i = 0; i < columns_to_reduce.size(); ++i) {
...
#endif
				// add pairs
				persistence_pairs.push_back(dim);
				persistence_pairs.push_back(diameter);
				persistence_pairs.push_back(R_PosInf);
		...
		found_persistence_pair:
			...
			if (diameter != death) {
			  ...
			  persistence_pairs.push_back(dim);
			  persistence_pairs.push_back(diameter);
			  persistence_pairs.push_back(death);
			}
...
Rcpp::NumericMatrix run_ripser(Rcpp::NumericVector dist_vec, 
  ...
  if (verbose){
    Rcpp::Rcout << "distance matrix with " << n << " points" << std::endl;
  }
  
  auto value_range = std::minmax_element(dist.distances.begin(), dist.distances.end());
  
  if (verbose){
    Rcpp::Rcout << "value range: [" << *value_range.first << "," << *value_range.second << "]" << std::endl;
  }
  ...
  
  {
    ...
    for (auto e : edges) {
      ...
      if (u != v) {
        if (get_diameter(e) > 0){
          persistence_pairs.push_back(0);
          persistence_pairs.push_back(0);
          persistence_pairs.push_back(get_diameter(e));
        }
  ...
  for (index_t dim = 1; dim <= dim_max; ++dim) {
    ...
    std::vector<double> dim_pairs = compute_pairs(columns_to_reduce, pivot_column_index, dim, n, threshold, modulus, multiplicative_inverse, dist,
                                                   comp, comp_prev, binomial_coeff);
    persistence_pairs.insert( persistence_pairs.end(), dim_pairs.begin(), dim_pairs.end() );
  ...
  Rcpp::NumericVector output = Rcpp::wrap(persistence_pairs);
  output.attr("dim") = Rcpp::Dimension(3, persistence_pairs.size()/3);
  Rcpp::NumericMatrix out = Rcpp::wrap(output);
  Rcpp::NumericMatrix pers = Rcpp::transpose(out);
  Rcpp::colnames(pers) = Rcpp::CharacterVector::create("dim", "birth", "death");
  return pers;
}
